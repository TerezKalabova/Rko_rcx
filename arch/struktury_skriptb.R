...
NKB$kody_K<-gsub("[[:space:]]", " ", NKB$kody_K)              #nahrazeni vsech typu mezer jednotno mezerou 1./3 krok
NKB$kody_K<-gsub("\xc2\xa0", " ", NKB$kody_K)                 #nahrazeni vsech typu mezer jednotno mezerou 2.3 krok
NKB$kody_K<-gsub(" ", " ", NKB$kody_K)                        #nahrazeni vsech typu mezer jednotno mezerou 3/3 krok
...
bez_v<-"chybi delitko parujici otazka-dotaznik"
...
for (cis in 1:nrow(NKB)) { #prace po radcich K koduu
 ...
 #chyby v kody_K, kdy se dale kody_K ani nezkousi analyzovat
  ...
  # -----------------------------------------------------------------------------------------------
    ...
    seznam_dotazniku<-c("E5", "S5", "F5", "T5", "PN18/2", "T2", "F6", "G8")
  ...
  # -----------------------------------------------------------------------------------------------
    # krajeni 1 kodu K na retezce
      ...
      useky_pocty_znaku_bez_mezer<-gsub(" ","",useky_mezi_carkami)    
      ...
      for (us in 1 : length(useky_mezi_carkami) ) { #prochazeni kody_K po usecich
        ...
        if (nchar(gsub(" ","", useky_mezi_carkami[us]))>=5 && grepl(" v ", useky_mezi_carkami[us])) { # usek ma min delku && obsahuje " v "
          nadejny_pahyl<-useky_mezi_carkami[us]
          nadej_pahyl_ret<-unlist(strsplit(nadejny_pahyl, " v "))
          if (length(nadej_pahyl_ret)==2 &&  is.element(gsub(" ","", nadej_pahyl_ret[2]), seznam_dotazniku) &&  !is.element(gsub(" ","", nadej_pahyl_ret[1]), prvky_skal) ) { # nadejny pahyl se rozpada prave na dve casti  && v nadejnem pahylu to za " v " ma typ seznam_dotazniku, to pred v neni prvek_skal
            ...
            pahyl_otazek<-paste(pahyl_otazek, useky_mezi_carkami[us], ", ", sep="")
        ...
        } else if (nchar(gsub(" ","", useky_mezi_carkami[us]))==4 && grepl(" v ", useky_mezi_carkami[us]) && is.element(substr(useky_mezi_carkami[us],1,1),prvky_skal) ) { #ma to jen 4 znaky, slkada se to z casti: prvek_skal-" v "-dotaznik
          #tady by se mely odrezat vsude mezery v ifu TODO
          
          #tady to ted hori!!!!! proto to nefunguje. cus v pondeli!
          a<-1
        } else { # TODO odstranit/ jen pro vyvoj ukazani chyby
          useky_predb_ok<-c(useky_predb_ok, "chyba nevim kde")
        }
        useky_ok<-c(useky_ok, useky_predb_ok) #pokud je to ok
      }#for u
      ...
      # a<-"a v v d"
        ...
        #nejjednodussi funkcni pripad
        if all(str_count(useky_mezi_carkami, " v ") == rep(1, length(useky_mezi_carkami)) ) {
          je to vsechno asi ok
        }
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        K_po_retezcich<-unlist(strsplit(kody_K, ", ")) #rozdeleni K kodu do retezcu po dotaznicich podle ", "
        ...
        for (dot in 1: length(K_po_retezcich) ) { #prace po retezcich z kodu K
          ...
          ret<-gsub(" ", "", unlist(strsplit(K_po_retezcich[dot], " v "))) #rozdeleni K kodu na otazku a dotaznik podle " v "
          OTAZKA<-ret[1]
          DOTAZNIK<-ret[2]
