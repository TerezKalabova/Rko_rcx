...
#-----
seznam_dotazniku <- toupper(as.vector(unlist(read.table("/home/kalabava/sya/60_Elspac/03_dokumenty_k_Elspacu/07_sjednoceni_kodovniku/02_input_data/seznam_dotazniku.txt", header=TRUE, sep=";"))))
...
skala_cisla<-seq(0,49)
skala_rim<-tolower(as.roman(seq(1:49)))
skala_pis<-letters[1:26]
prvky_skal<-c(skala_cisla, skala_rim, skala_pis)
typy_skal<-c(rep("cisla",50), rep("rim", 49), rep("pis", 26))
...
#kontrolni vypis obsahu listu NK2 do ulozitelne tabulky
NK2tab<-c()
...
for (cis in 1:nrow(NKB)) { #prace po radcich K koduu
 ...
 #chyby v kody_K, kdy se dale kody_K ani nezkousi analyzovat
  ...
  #V 2i v V13, V 2i v V8, V 11
  #kody_K<-"N 20-23a, N24b, N25a, N26a v NT9"
  #kody_K<-"J 2-4j1,2 , J 6,7 ji1,2 v FT4, H 1j , H 2j v S 4, J 1i-iiij , J 2i-iij v E4, J1i-iiij, J2i-iij v TH4"
        ...
        if (is.null(useky_ok)) { #retezec se nepovedlo rozdelit ani na jeden validni usek --> chyba ##
          ...
          list_K<-as.data.frame(cbind(chyba_ret, "chyba", "chyba"))
            ...
            for (dot in 1: length(K_po_retezcich) ) { #prace po retezcich z kodu K
              ...
              # -------------------------------------------------------------------------------------------------
              #mnozeni otazek pro carku a pomlcku
              # -------------------------------------------------------------------------------------------------
              
              if (grepl(",",OTAZKA) || grepl("-",OTAZKA)) { #podminka / OTAZKA obsahuje aspon jednu carku nebo pomlcku
                
                # pocet velkych pismen vretezci OTAZKA, tj pocet unikatnich prvnich pahylu
                # osetruje spravne rozsirovani moznosti do vsech predeslych moznosti
                ind_mnozeni_otazky<-as.vector(gregexpr('[[:upper:]]', OTAZKA)[[1]])
                pocet_prv_pah<- length(ind_mnozeni_otazky) #funguje jen na prvni prvek vektoru, kdybych chtela scitat 
                
                #retezce pro mnozeni otazky
                ...
                finpodot<-c()
                for (mnozeni_otazky in 1:pocet_prv_pah) { 
                  podotmnoz<-ret_mnozeni_otazky[mnozeni_otazky]
                  
                  #otazka s carkami =======================================================================================================================================
                  if (grepl(",",podotmnoz)) { #zmnozena otazka na 1 dotaznik / pokud jsou v otazce carky, otazka se rozdeli do vice podotazek
                    podotmnoz<-unlist(strsplit(podotmnoz, ",")) 
                      ...
                      if (length(notUpperInd) >0) {                   
                        if (min(notUpperInd)>1) { #je druhy a vys
                            ...
                            for (u in 1 : length(notUpperInd) ) {
                              usek_s_malym<-podotmnoz[notUpperInd[u]] #usek s malym
                              upredpodot<-c()
                              for (upred in 1:length(pompodotmnoz)) {
                                usek_pred<-pompodotmnoz[upred]
                                     
                                zackon<-c(usek_pred, usek_s_malym)
                                posledni_znaky_ot<-c(str_sub(zackon[1], -1,-1), str_sub(zackon[1], -2,-1), str_sub(zackon[1], -3,-1), str_sub(zackon[1], -4,-1), str_sub(zackon[1], -5,-1))
                               ...
                               if (all(typy_zackon=="cisla")) {
                                  ...
                                  skala<-skala_rim
                                } else if ((length(typy_zackon)==2) && typy_zackon[1]!=typy_zackon[2]) {  #je tam chyba ve skale: 
                                  ...
                                  list_K<-as.data.frame(cbind(chyba_skala, "chyba", "chyba")) #vypise chybu do listu cis a skace na dalsi kolo forcyklu
                               } else { #chyba ve skale nevim
                                  NK_cis_chyba_kody_K<-rbind(NK_cis_chyba_kody_K , cbind(NKB[cis,], chyba=chyba_skala_nevim)) #vypise chybu do listu cis a skace na dalsi kolo forcyklu
                               ...
                               if (is.null(chyba_uvnitr_kodu)) { #toto osetruje zatim jen chybu v moznostech skaly (17.6.2015), pokud je to TRUE, neni tam chyba.
                                  ...
                                  pompodotmnoz<-paste(chyba_uvnitr_kodu, ": ", OTAZKA, sep="")
                                }
                              } #for upred
                              pompodotmnoz<-sort(upredpodot)
                              ###OTAZKA<-pompodotmnoz ####  
                            } #for u
                        } else {  
                          NK_cis_chyba_kody_K<-rbind(NK_cis_chyba_kody_K , cbind(NKB[cis,], chyba="otazka_ma_velke_pismeno_jinde_nez_na_zacatku")) #zapise cely chybny radek z NKB do tabulky chyb
                          chyba_uvnitr_kodu<-"otazka_ma_velke_pismeno_jinde_nez_na_zacatku"
                        }
                ...
                OTAZKA<-finpodot
              } #if OTAZKA obsahuje aspon jednu carku nebo pomlcku 
              ...
              } else if (grepl("[[:punct:]]", gsub(",","",(gsub("/","",DOTAZNIK))) ) ) { #v dotazniku jsou zvlastni znaky !"#$%&'()*+-.:;<=>?@[]^_`{|}~.,
                DOTAZNIK<-paste("neznama_chyba_dotaznik: ", DOTAZNIK, sep="")
                NK_cis_chyba_kody_K<-rbind(NK_cis_chyba_kody_K , cbind(NKB[cis,], chyba="zvlastni_znaky_v_dotazniku"))
              }
              ...
              #fin kontrola dotaznikuu (18.8.2015): if vsechny prvky dotaznik parti do seznamu dotazniku pak provest kod "DOTAZNIK<-DOTAZNIK" a je to ok, pokud ne, tak vyhodit chybu
              if (all(is.element(as.vector(DOTAZNIK), as.vector(seznam_dotazniku)))) { #prvky dotazniku match seznam dotazniku) 
                    DOTAZNIK<-DOTAZNIK  #je to tu navic, ale nechtela jsem nechat telo cyklu prazdne.
              } else {
                DOTAZNIK<-"chyba_nedef_kod_dotazniku"
                NK_cis_chyba_kody_K<-rbind(NK_cis_chyba_kody_K , cbind(NKB[cis,], chyba="chyba_nedef_kod_dotazniku"))
              }
            ...
            kody_K_lower<-unique(na.omit(unlist(strsplit(kontrola_znaku_kody_K, "[^a-z]"))))
            kontrola_znaku_finkody<-sort(unique(unlist(strsplit(c(otazky,dotazniky), ""))))
            ...
            if (!(all(is.element(kody_K_cisla, finkody_K_cisla)) && all(is.element(kody_K_upper, finkody_K_upper)) &&  all(is.element(kody_K_lower, finkody_K_lower)) )) { #neobsahuje vsechny druhy velkych pismen, malych pismen a cisel jako na zacatku
              #zapise cely chybny radek z NKB do tabulky chyb
              NK_cis_chyba_kody_K<-rbind(NK_cis_chyba_kody_K , cbind(NKB[cis,], chyba="kontrola_ztracenych_znaku"))
              #vypise chybu do listu cis a skace na dalsi kolo forcyklu
              list_K<-as.data.frame(cbind("kontrola_ztracenych_znaku", "chyba", "chyba"))
            } else if (any(str_sub(otazky, 1, 1) != toupper(str_sub(otazky, 1, 1)))) { #odchyceni otazek zacinajich malym pismenem (nejspis spatne rozdelene)
              #zapise cely chybny radek z NKB do tabulky chyb
  ...
  #znovuulozeni nazvu sloupcu i pro chybove vypisy, kde bylo jen V1, V2, V3
  names(list_K)<-c("otazky", "dotazniky", "otazkyDB")
 
  NK2[[cis]]<-list_K
  # ulozeni do kontrolni tabulky
  NK2tab<-rbind(NK2tab , cbind(list_K, as.vector(NKB[cis,])))
